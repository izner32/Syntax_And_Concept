...
def sampleArgs(
        *args):  # this is not a pointer, this is an args which means you're passing multiple arguments. This is the equivalent of '...' in c++
    print(args)
...
def sampleKwargs(
        **kwargs):  # this is not a pointer to a pointer, the purpose of this is the same for *args but explicitly for keywords
    print(kwargs)
...
def sampleListArg(
        list):  # idk if only the first element is getting passed and it's just getting incremented (list[i]) but i think it is since this is the case for other programming languages
    for x in list:
        print(x)
...
class SampleClass:
    ...
    def __init__(self, firstName, lastName,
                 age):  # self is the equivalent of 'this' and it is required as parameter when using the functionality of 'this'
...
if result1:
    print("first whitespace is located at position: ",
          result1.start())  # with (,) comma inside print, you explicitly convert int to str so it is much better to use than '+'
...
listSample[1:3] = [20, 30,
                   50]  # you are changing the 2nd index element up to 4th index element, also impossible in c++ and java
...
listSample.pop(
    0)  # we have removed the first index element, leaving the argument blank would mean remove the last item of the list
del listSample[
    0]  # remove the first index element, 'del listSample' means deleting all of the content of the list or the list itself?
...
[print(x) for x in
 loopThisList]  # very weird way to loop but it is a one liner, it is also called as list comprehension

# LESSON 7.72 - LIST COMPREHENSION | this is a one liner for loop
listCompre = [1, 2, 3, 4, 5, 6, 7]
[print(x) for x in
 listCompre]  # syntax: content for var in listToLoop | you could also do double for loop just add another for var in listToLoop
...
print(
    sampleDictionary.items())  # idk really what this does but it seems to convert everything into an item of each on their own including the keys
...
class SampleDunderClass:
    ...
    def __repr__(
            self):  ##this is displayed to the end user when an object of this class is called but the display is overriden when u use __str__, this is meant for developers
...
print(funcFunc000(
    funcFunc))  # here because we have written functionPass = funcFunc, functionPass becomes a reference to function funcFunc, if we wanted to grab it by value then we must do functionPass = funcFunc()
